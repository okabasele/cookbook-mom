...
import { LanguageAvailable } from '@/types/Recipe';

const youtubeUrlPattern =
  /^(https?:\/\/)?(www\.)?(youtube\.com|youtu\.be)\/.+$/;
const youtubeIdPattern = /(?:youtube\.com\/watch\?v=|youtu\.be\/)([0-9A-Za-z_-]{11})/;
const isRecipeVideo = (title: string, description: string): boolean => {
...
export const retrieveYoutubeVideoRecipe = async (
  url: string,
  targetLang: LanguageAvailable
): Promise<{
  title: string;
  language: string;
  ingredients: string[];
